#ifndef _ROS_cob_object_detection_msgs_DetectionArray_h
#define _ROS_cob_object_detection_msgs_DetectionArray_h
...
namespace cob_object_detection_msgs
{
  ...
  class DetectionArray : public ros::Msg
  {
    public:
      ...
      cob_object_detection_msgs::Detection st_detections;
      cob_object_detection_msgs::Detection * detections;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(detections_lengthT > detections_length)
        this->detections = (cob_object_detection_msgs::Detection*)realloc(this->detections, detections_lengthT * sizeof(cob_object_detection_msgs::Detection));
      detections_length = detections_lengthT;
      for( uint32_t i = 0; i < detections_length; i++){
      offset += this->st_detections.deserialize(inbuffer + offset);
        memcpy( &(this->detections[i]), &(this->st_detections), sizeof(cob_object_detection_msgs::Detection));
      }
     return offset;
    }

    const char * getType(){ return "cob_object_detection_msgs/DetectionArray"; };
